...
int silz = 0;
        silz = log10(g)+1;    
char *buffx ;       // Buffx's strlen() cannot be longer  than Buffy
buffx = malloc (sizeof (char)*silz+1);
    sprintf(buffx, "%d", g);
...
while (o < gig){ *(spcline+o) = o; o++;}    
  ...
  if (strlen(final) == 1){printf("%s",final);
                          g++;
                          buffy = malloc ( sizeof(char)* (strlen(final)+1) );
                          buffy[0] = final[0]; buffy[1] = final[1];
  ...
  buff = Listerate(nd, buff,bl+1);  
    printf("%s\n", buff);
    g++;
    buffy = malloc ( sizeof(char)* (strlen(buff)) );
    for (int yy = 0; yy < strlen(buff); yy++){ buffy[yy] = buff[yy]; }
    Listend(nd);
    }
