...
int main()
{
 ...
 char *sounds = "pong-ping-bink-pong-ping-donk-pong-ping-pong-bangping-pong-bink-ping-pong-donk-pong-ping-pong-ping-bangping-pong-aaaa-ping-pong-ping-aaaapong-ping-pong-ping-aaaa-pong-ping-pong-aaaa";
 unsigned bin =0; 
  unsigned bon =0;
  ...
  for (int i = 0; i<game; i++){
    if(sounds[i-1] == '-' ){ 
      ...
      if (sounds [i-5] == 'p' && sounds[i-4] =='i' ){ 
          ...
          for (int x =0; x < strlen(ponk)-1; x++){
                    if (sounds[i+x] != ponk[x]){
                                                offs[nxt] = i-1;
                                                nxt++;
                                                offs[nxt +1] = '\0';
                                                printf("%c ", sounds[i-1]);break;   }
        }
      }
      if (sounds [i-5] == 'p' && sounds[i-4] =='o' ){
          for (int y =0; y < strlen(pink)-1; y++){
          if (sounds[i+y] != pink[y]){
                                                offs[nxt] = i-1;
                                                nxt++;
                                                offs[nxt +1] = '\0';
                                                printf("%c ", sounds[i-1]);break;   }
