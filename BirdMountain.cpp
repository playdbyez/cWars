...
"^^^^^^^^^^^^^^^^                             ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^",
  ...
  int inc =1;
   ...
   for (size_t g = 0; g < mountain.size(); ++g){ if (dn0 == 1){break;}
    ...
    //if (bot == tob) {level --;}
     if (dn0 == 1){   for(size_t v = 0; v< mountain[tob].size();v++){if (mountain[tob][v] == '^' ){if (level == '@'){cout << "tob found on line " << tob << "\n";}mountain[tob][v] = level;}}}
    
    if (dn1 == 1){   for(size_t w = 0; w< mountain[bot].size();w++){if (mountain[bot][w] == '^' ){if (level == '@'){cout << "bot found on linex " << bot << "\n";}mountain[bot][w] = level;}}}
...
{
   //Expected 11
   //This one is right because although eleven is only one square wide it doesnt repeat itself
vector<string> mountain = {
"^^^^^^^^^^^^^^^^^^^^^",
"^^^^^^^^^^^^^^^^^^^^^",
"^^^^^^^^^^^^^^^^^^^^^",
...
"^^^^^^^^^^^^^^^^^^^^^",
  ...
  int inc =1;
   ...
   for (size_t g = 0; g < mountain.size(); ++g){ if (dn0 == 1){break;}
    ...
    //if (bot == tob) {level --;}
     if (dn0 == 1){   for(size_t v = 0; v< mountain[tob].size();v++){if (mountain[tob][v] == '^' ){if (level == ';'){cout << "tob found on line " << tob << "\n";}mountain[tob][v] = level;}}}
    if (level == ';'){cout << "bot found on linex " << bot << "\n";}
